...
- If running inside `/full-pass run`, receive audit results from the steward's parallel dispatch output
...
5. **Decision points:** For each contradiction, articulate what's at stake. When Victore says "be braver" and Peers says "this is already strong," Peter needs to decide which lens to favor for that specific element.
...
- What Peter gains by favoring Lens A (and what he loses)
- What Peter gains by favoring Lens B (and what he loses)
- A recommendation IF one lens clearly aligns with the positioning intent ("desirable because interesting and unavailable")

## Called By

- `/full-pass run` — Step 3.5, runs sequentially AFTER audit returns (needs audit output as input)
